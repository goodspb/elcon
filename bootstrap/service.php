...
$config = is_file($configFile = dirname(__DIR__) . '/app/common/config/production/service.php') ? include($configFile) :
    [];
$runDir = isset($config['run_dir']) ? $config['run_dir'] : '/tmp/glife/';
...
$request = [
    'request' => $_REQUEST, 'cookies' => $_COOKIE, 'server' => $_SERVER, 'files' => $_FILES,
];
